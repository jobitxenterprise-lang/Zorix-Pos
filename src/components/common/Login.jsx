...
export const Login = () => {
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4 font-sans">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md shadow-xl overflow-hidden">
        {/* Header del login */}
        <div className="pt-10 pb-4 text-center px-8">
          <div className="w-56 h-40 flex items-center justify-center mx-auto">
           <img src={logo} alt="Zorix POS" className="object-contain max-h-full" />
          </div>
        </div>
        {/*Cabezera del step */}
        <h1 className="text-center text-2xl font-bold text-blue-950 m-0">
          {step === 1
        ...
        {step === 1 ? (
          <div className="space-y-3">
            <button
              ...
              className="w-full mt-2 p-4 rounded-2xl text-center border border-slate-200 bg-slate-50 hover:bg-blue-300 hover:border-blue-400 shadow-sm transition-all cursor-pointer group"
            >
              <div>
                <h3 className="font-bold text-blue-950 text-sm">Administrador</h3>
              </div>
            </button>
            <button
              type="button"
            ...
            <button
              ...
              className="w-full p-4 rounded-2xl border border-slate-200 bg-slate-50 hover:bg-blue-300 hover:border-blue-400 shadow-sm transition-all text-center cursor-pointer group"
            >
              <div>
                <h3 className="font-bold text-blue-950 text-sm">Mesero</h3>
              </div>
            </button>
          </div>
        ) : (
          <form onSubmit={handleFormSubmit} className="space-y-5 animate-in slide-in-from-right-4 duration-300">
            ...
            {loginType === "mesero" || loginType == 'cajero'? (
              ...
              <div className="space-y-4 pt-2">
                <div>
                  <label className="block text-xs font-bold text-blue-950 mb-2">Usuario</label>
                  <input
                    type="text"
                    required
                    value={username}
                    onChange={(e) => setUsername(e.target.value)}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-blue-950 text-sm focus:outline-none focus:border-blue-950 transition-colors font-medium"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-blue-950 mb-2">Contraseña</label>
                  <input
                    type="password"
                    required
                    value={password}
                    onChange={(e) => setPassword(e.target.value)}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-blue-950 text-sm focus:outline-none focus:border-blue-950 transition-colors font-mono"
                  />
                </div>
              </div>
            )}
            <div className="flex gap-3 pt-4">
              <button
                ...
                className="px-4 py-3.5 bg-slate-100 hover:bg-blue-300 text-slate-700 hover:text-blue-950 font-bold text-sm rounded-xl transition-all cursor-pointer border border-slate-200"
              >
                Atrás
              </button>
              <button
                type="submit"
                className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-blue-950 hover:bg-blue-900 text-white font-bold text-sm rounded-xl transition-all shadow-md cursor-pointer"
              >
